...
По-умолчанию в любом новом репозитории GitHub предлагается создать пустой файл `README.md`. Он явдяется главной страницей репозитория (аналог `index.html`) и автоматически отображается при открытии репозитория в брайзере.
...
Ещё однин способ разметить заголовки:
...
Для полужирного курсива используется обрамление тройными симолами `***` или `___` (можно в перемешку).
...
```
[Относительная ссылка на этот же раздел](Markdown-Syntax.md#Ссылки)
```
...
Чтобы быстро вставить ссылку в Visual Studio Code, нужно написать её заголовок, выделить его и нажать <kbd>Ctrl</kbd> + <kbd>V</kbd>. Трубуется установленный плагин Markdown All in One.
...
В чистом Маркдауне нет синтаксиса для таблиц, а в  Github Flavored Markdown есть.
...
Чтобы быстро отформатировать таблицу в Visual Studio Code, нужно выделить её и нажать <kbd>Alt</kbd> + <kbd>Shift</kbd> + <kbd>F</kbd>. Трубуется установленный плагин Markdown All in One.
...
<details>
  ...
  Текст спойлера. Обратите внимание, что после тега `</summary>` должена быть пустая строка. Можно писать любой Markdown внутри тега `<details>`, только
  убедитесь, что вы закрыли `</details>` после этого.
