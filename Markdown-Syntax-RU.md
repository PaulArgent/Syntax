...
По-умолчанию в любом новом репозитории GitHub предлагается создать пустой файл `README.md` для описания репозитория. Данный файл является главной страницей репозитория (аналог `index.html` в HTML-разметке) и автоматически отображается при открытии репозитория в браузере.
